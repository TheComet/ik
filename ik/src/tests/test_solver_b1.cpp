...
TEST_F(NAME, rotate_90_degrees_initial_rotations)
{
    ...
    tip->position = ik_vec3(sqrt(2), sqrt(2), 0);
    ik_quat_set_axis_angle(tip->rotation.f, 0, 0, 1, M_PI/4);
    e->target_position = ik_vec3(0, -sqrt(8), 0);
    ...
    // Tip position should remain the same in local space
    EXPECT_THAT(tip->position.v.x, DoubleEq(sqrt(2)));
    EXPECT_THAT(tip->position.v.y, DoubleEq(sqrt(2)));
    EXPECT_THAT(tip->position.v.z, DoubleEq(0));

    // Base should have rotated about the Z axis by 90°
    union ik_quat q; ik_quat_set_axis_angle(q.f, 0, 0, 1, M_PI/4 - M_PI/2);
    EXPECT_THAT(base->rotation.q.x, DoubleEq(q.q.x));
    EXPECT_THAT(base->rotation.q.y, DoubleEq(q.q.y));
    EXPECT_THAT(base->rotation.q.z, DoubleEq(q.q.z));
    EXPECT_THAT(base->rotation.q.w, DoubleEq(q.q.w));

    // Tip rotation should remain identical
    ik_quat_set_axis_angle(q.f, 0, 0, 1, M_PI/4);
    EXPECT_THAT(tip->rotation.q.x, DoubleEq(q.q.x));
    EXPECT_THAT(tip->rotation.q.y, DoubleEq(q.q.y));
    EXPECT_THAT(tip->rotation.q.z, DoubleEq(q.q.z));
    EXPECT_THAT(tip->rotation.q.w, DoubleEq(q.q.w));
}
