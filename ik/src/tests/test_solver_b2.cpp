...
TEST_F(NAME, reach_target_coplanar_segments)
{
    ...
    base->position = ik_vec3(1, 2, 1);
    mid->position = ik_vec3(0, 2, 0);
    tip->position = ik_vec3(2, 0, 0);
    e->target_position = ik_vec3(0, -2, 0);
    ...
    EXPECT_THAT(tip->position.v.y, DoubleEq(0));
    EXPECT_THAT(tip->position.v.z, DoubleEq(0));
...
TEST_F(NAME, reach_target_no_joint_rotations)
{
    ...
    // Tip position should remain the same in local space
    EXPECT_THAT(tip->position.v.x, DoubleEq(1));
    EXPECT_THAT(tip->position.v.y, DoubleEq(-sqrt(3)));
    EXPECT_THAT(tip->position.v.z, DoubleEq(0));
    ...
    EXPECT_THAT(base->rotation.q.w, DoubleEq(1));

    // Mid should have rotated about the Z axis by 60°
    EXPECT_THAT(mid->rotation.q.x, DoubleEq(0));
    EXPECT_THAT(mid->rotation.q.y, DoubleEq(0));
    EXPECT_THAT(mid->rotation.q.z, DoubleEq(0));
    EXPECT_THAT(mid->rotation.q.w, DoubleEq(1));
