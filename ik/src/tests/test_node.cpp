...
{
    ik_node* n = ik_node_create(ik_guid(3));
...
#endif
{
    ...
    ik_node* n1 = ik_node_create(ik_guid(2));
    ik_node* n2 = ik_node_create_child(n1, ik_guid(3));
    ik_node* n3 = ik_node_create_child(n2, ik_guid(2));
    /*
#ifdef DEBUG
...
TEST_F(NAME, error_if_child_nodes_have_same_guid)
{
    ik_node* n1 = ik_node_create(ik_guid(2));
    ik_node* n2 = ik_node_create_child(n1, ik_guid(3));
    ik_node* n3 = ik_node_create(ik_guid(3));

    // Test both link and create_child
    EXPECT_THAT(ik_node_link(n1, n3), Eq(IK_ERR_DUPLICATE_NODE));
    ik_node* n4 = ik_node_create_child(n1, ik_guid(3));
    EXPECT_THAT(n4, IsNull());
    IK_DECREF(n1);
    IK_DECREF(n3);
}
...
TEST_F(NAME, child_keeps_parent_if_link_fails)
{
    ...
    ik_node* n3 = ik_node_create(ik_guid(2));
    ik_node* n4 = ik_node_create_child(n3, ik_guid(3));

    EXPECT_THAT(n4->parent, Eq(n3));
    EXPECT_THAT(ik_node_link(n1, n4), Eq(IK_ERR_DUPLICATE_NODE));
    EXPECT_THAT(n4->parent, Eq(n3));
...
TEST_F(NAME, find_includes_root_node)
{
    ik_node* n1 = ik_node_create(ik_guid(2));
    ik_node* n2 = ik_node_create_child(n1, ik_guid(3));
    ik_node* n3 = ik_node_create_child(n2, ik_guid(4));
...
TEST_F(NAME, find_child_from_root)
{
    ...
    ik_node* n2 = ik_node_create_child(n1, ik_guid(3));
    ik_node* n3 = ik_node_create_child(n2, ik_guid(4));
    ik_node* n4 = ik_node_create_child(n2, ik_guid(5));
...
TEST_F(NAME, find_doesnt_find_parents)
{
    ...
    ik_node* n2 = ik_node_create_child(n1, ik_guid(3));
    ik_node* n3 = ik_node_create_child(n2, ik_guid(4));
    ik_node* n4 = ik_node_create_child(n1, ik_guid(5));
    EXPECT_THAT(ik_node_find(n1, ik_guid(5)), Eq(n4));
...
{
    ik_node* n1 = ik_node_create(ik_guid(1));
    ik_node* n2 = ik_node_create(ik_guid(2));
