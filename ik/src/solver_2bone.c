...
solver_2bone_construct(ik_solver_t* solver)
{
    ...
    solver->destruct = solver_2bone_destruct;
    solver->post_chain_build = solver_2bone_post_chain_build;
    solver->solve = solver_2bone_solve;
...
solver_2bone_post_chain_build(ik_solver_t* solver)
{
    ...
    SOLVER_FOR_EACH_BASE_CHAIN(solver, base_chain)
        if (chain_length(base_chain) != 3) /* 3 nodes = 2 bones */
        {
...
solver_2bone_solve(ik_solver_t* solver)
{
    SOLVER_FOR_EACH_BASE_CHAIN(solver, base_chain)
        ...
        node_mid  = chain_get_node(base_chain, 1);
        node_base = chain_get_node(base_chain, 2);
