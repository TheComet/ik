...
chain_construct(struct ik_chain_t* chain)
{
    ...
    ordered_vector_construct(&chain->children, sizeof(struct ik_chain_t));
}
...
{
    chain_destruct(chain); /* does the same thing as destruct */
}
...
{
    ORDERED_VECTOR_FOR_EACH(&chain->children, struct ik_chain_t, child_chain)
...
/* ------------------------------------------------------------------------- */
static int
count_chains_recursive(struct ik_chain_t* chain)
{
    int counter = 1;
...
#if IK_DOT_OUTPUT == ON
static void
dump_chain(struct ik_chain_t* chain, FILE* fp)
{
...
void
dump_to_dot(struct ik_node_t* node, struct ik_chain_t* chain, const char* file_name)
{
...
/* ------------------------------------------------------------------------- */
static int
mark_involved_nodes(struct ik_solver_t* solver, struct bstv_t* involved_nodes)
{
    /*
...
/* ------------------------------------------------------------------------- */
static int
recursively_build_chain_tree(struct ik_chain_t* chain_current,
                             ...
                             struct bstv_t* involved_nodes)
{
    ...
    struct ik_chain_t* child_chain = chain_current;
